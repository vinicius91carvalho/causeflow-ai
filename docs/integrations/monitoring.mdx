...
<Tabs>
  ...
  <Tab title="CloudWatch">
    ...
    **Webhook URL:** `https://api.causeflow.ai/v1/webhooks/{tenantId}/cloudwatch`
    ...
    <Tip>
      A sample Lambda proxy template is available in the [CauseFlow examples repository](https://github.com/causeflow/examples). It adds the required `X-API-Key` header to every SNS notification before forwarding to CauseFlow.
    </Tip>
