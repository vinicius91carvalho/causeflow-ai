...
| `CLERK_SECRET_KEY` | Clerk secret key (server-only — drives `clerkClient()`) |
| `NEXT_PUBLIC_CLERK_SIGN_IN_URL` / `_SIGN_UP_URL` / `_AFTER_SIGN_IN_URL` / `_AFTER_SIGN_UP_URL` | Clerk routing overrides |
| `CORE_API_URL` | Base URL for the Core API — `https://api-staging.causeflow.ai` on staging. Blank in local dev falls through to the mock client. |
| `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Stripe publishable key (safe for client). Checkout URLs are generated by the Core API, so the dashboard does **not** hold `STRIPE_SECRET_KEY`. |
| `AWS_REGION` | Only used by CLI scripts under `scripts/` that talk to the Core API's DynamoDB directly (e.g. `users:delete`). Not used at runtime. |
| `NEXT_PUBLIC_GA4_MEASUREMENT_ID` / `NEXT_PUBLIC_CLARITY_ID` | Analytics IDs |
...
All runtime vars injected by SST at deploy time. For local dev, use `apps/dashboard/.env.local`. CLI scripts may additionally require `STRIPE_SECRET_KEY` / `DYNAMODB_TABLE_NAME` — these come from the Core repo's `.env.local` or AWS SSO, not from the dashboard's runtime environment.

## Stripe / Billing

Stripe is primarily owned by the **Core API**, not the dashboard:

- Checkout sessions are created by the Core API (`POST /v1/billing/checkout`) — the dashboard only forwards the request via `api.createCheckout()`.
- Customer Portal sessions are created by the Core API (`POST /v1/billing/portal`).
- Stripe webhooks point at the Core API; the Core API mirrors subscription state onto the `Tenant` entity (`stripeCustomerId`, `stripeSubscriptionId`, `subscriptionStatus`, `currentPeriodEnd`).
- The dashboard reads subscription state via `GET /v1/billing/subscription` and computes the gate in `contexts/billing/application/plan-status.ts`: a plan is "real" only if `status ∈ {active, trialing}` **and** `stripeCustomerId` is set.
- The `stripe` npm package is installed as a dev dependency only — used by CLI scripts under `scripts/` (e.g. `stripe:setup`, `users:delete`) and NOT at runtime. `STRIPE_SECRET_KEY` does not belong in the dashboard's runtime env.
...
# Set up Stripe products and prices programmatically (run once per Stripe account)
pnpm --filter dashboard run stripe:setup

# Completely purge a user from Clerk + Stripe + the Core API's DynamoDB table.
# Deletes the user, their Clerk organization(s), the Stripe customer (which cancels subscriptions),
# and every DynamoDB item scoped to that tenantId. Destructive — pass --yes to actually run.
pnpm --filter dashboard run users:delete -- --email <email>[,email2,...] --stage staging --yes
```

Scripts live in `apps/dashboard/scripts/`. Run from the project root using `pnpm --filter dashboard run <script>`.
