...
export class SlackNotificationSubscriber {
    ...
    async onIncidentCreated(event: DomainEvent): Promise<void> {
        try {
            ...
            if (!tenant?.settings?.slackConfig) {
                this.logger.info({ tenantId, incidentId }, 'slack.notification.skipped reason=not_configured');
                return;
            }
            ...
            // Deduplication check
            const existing = await this.slackNotificationRepo.findNotification(tenantId, incidentId, 'alert');
            if (existing) {
                this.logger.info({ tenantId, incidentId }, 'slack.notification.skipped reason=dedup');
                return;
            }

            const incident = await this.incidentRepo.findById(toTenantId(tenantId), toIncidentId(incidentId));
            if (!incident) {
                this.logger.warn({ tenantId, incidentId }, 'slack.notification.skipped reason=incident_not_found');
                return;
            }
            ...
            this.logger.info({ tenantId, incidentId, channel: slackConfig.channelId }, 'slack.notification.sent');
        } catch (err) {
            const incidentId = (event.payload['incidentId'] as string) ?? '';
            this.logger.error(
    ...
    async onInvestigationStarted(event: DomainEvent): Promise<void> {
        try {
            ...
            const existing = await this.slackNotificationRepo.findNotification(tenantId, incidentId, 'investigation_started');
            if (existing) {
                this.logger.info({ tenantId, incidentId }, 'slack.notification.skipped reason=dedup');
                return;
            }
            ...
            this.logger.info({ tenantId, incidentId, channel: slackConfig.channelId }, 'slack.investigation_started.sent');
        } catch (err) {
            const incidentId = (event.payload['incidentId'] as string) ?? '';
            this.logger.error(
                { incidentId, error: err instanceof Error ? err.message : 'unknown' },
    ...
    async onInvestigationCompleted(event: DomainEvent): Promise<void> {
        try {
            ...
            const recommendedActions = (event.payload['recommendedActions'] as Array<{ description?: string; action?: string }>) ?? [];
            ...
            // Deduplication check
            const existing = await this.slackNotificationRepo.findNotification(tenantId, incidentId, 'resolution');
            if (existing) {
                this.logger.info({ tenantId, incidentId }, 'slack.notification.skipped reason=dedup');
                return;
            }
            ...
            this.logger.info({ tenantId, incidentId, channel: slackConfig.channelId }, 'slack.resolution.sent');
        } catch (err) {
            const incidentId = (event.payload['incidentId'] as string) ?? '';
            this.logger.error(
                { incidentId, error: err instanceof Error ? err.message : 'unknown' },
