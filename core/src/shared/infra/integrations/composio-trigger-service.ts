...
const TRIGGER_CATALOG: Record<string, AvailableTrigger[]> = {
    github: [
        ...
        { slug: 'GITHUB_PULL_REQUEST_EVENT', app: 'github', description: 'Pull request opened, merged, or closed' },
    ],
    sentry: [
        { slug: 'SENTRY_NEW_ISSUE', app: 'sentry', description: 'New issue detected' },
    ],
    pagerduty: [
...
async function getEnrichedCatalog(): Promise<Record<string, AvailableTrigger[]>> {
    ...
    try {
        ...
        logger.warn({ err: err instanceof Error ? err.message : err }, 'Failed to enrich trigger catalog from Composio');
        return TRIGGER_CATALOG;
    }
}
export class ComposioTriggerService {
    baseUrl = 'https://backend.composio.dev/api/v3';
    async registerWebhookSubscription(webhookUrl: string): Promise<{ registered: boolean; url: string }> {
        const apiKey = config.composio.apiKey;
        ...
        try {
            ...
            if (getRes.ok) {
                ...
                const items = data.items ?? (Array.isArray(data) ? data as Array<{ webhook_url?: string }> : []);
    ...
    async createTrigger(connectedAccountId: string, triggerSlug: string, triggerConfig: Record<string, unknown>, provider?: string): Promise<ComposioTriggerResult> {
        // Webhook-only providers (sentry, pagerduty, datadog) have no Composio trigger types.
        ...
        const res = await fetch(`${this.baseUrl}/trigger_instances/${encodeURIComponent(triggerSlug)}/upsert`, {
            method: 'POST',
            headers: {
                'X-API-KEY': apiKey,
