...
async function dispatchFargateTask(
    params: FargateTaskParams,
    ...
    if (failures.length > 0) {
        ...
        throw new Error(
            `Fargate task dispatch failed: ${failures[0]?.reason ?? 'unknown'}`,
        );
    }
...
export async function dispatchInvestigation(
    ...
    const environmentOverrides = [
        ...
        ...(config.aws.sqsEndpoint
            ? [{ name: 'SQS_ENDPOINT', value: config.aws.sqsEndpoint }]
            : []),
        ...
        ...(config.sts.roleArn
            ? [{ name: 'AWS_ROLE_ARN', value: config.sts.roleArn }]
            : []),
        ...(config.sts.stsEndpoint
            ? [{ name: 'STS_ENDPOINT', value: config.sts.stsEndpoint }]
            : []),
        ...
        // KMS
        ...(config.kms.endpoint
            ? [{ name: 'KMS_ENDPOINT', value: config.kms.endpoint }]
            : []),
        {
