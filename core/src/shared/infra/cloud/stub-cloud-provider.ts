import type {
  ...
  ServiceInfo,
  ResourceAction,
} from '../../application/ports/cloud-provider.port.js';
...
export class StubCloudProvider implements CloudProvider {
  ...
  async queryLogs(_creds: CloudCredentials, query: LogQuery): Promise<LogEntry[]> {
    ...
    const scenarios = {
      cpu: [
        {
          timestamp: ts(now, -10),
          message: `[${service}] CPU usage spike detected: 95.2%`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -9),
          message: `[${service}] Thread pool exhausted, request queue growing`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -8),
          message: `[${service}] GC pause: 1200ms (threshold: 200ms)`,
          level: 'warn',
          service,
        },
        {
          timestamp: ts(now, -7),
          message: `[${service}] Scaling event triggered: desired=4 running=2`,
          level: 'info',
          service,
        },
        {
          timestamp: ts(now, -5),
          message: `[${service}] Health check failed: timeout after 30s`,
          level: 'error',
          service,
        },
      ],
      oom: [
        {
          timestamp: ts(now, -12),
          message: `[${service}] Memory usage: 89% (4.2GB / 4.7GB)`,
          level: 'warn',
          service,
        },
        {
          timestamp: ts(now, -10),
          message: `[${service}] Memory usage: 95% (4.5GB / 4.7GB)`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -8),
          message: `[${service}] OOMKilled: container exceeded memory limit`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -7),
          message: `[${service}] Container restarting (exit code 137)`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -5),
          message: `[${service}] Service recovered after restart`,
          level: 'info',
          service,
        },
      ],
      timeout: [
        {
          timestamp: ts(now, -15),
          message: `[${service}] Connection to database timed out after 5000ms`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -14),
          message: `[${service}] Retry attempt 1/3 for db connection`,
          level: 'warn',
          service,
        },
        {
          timestamp: ts(now, -13),
          message: `[${service}] Retry attempt 2/3 for db connection`,
          level: 'warn',
          service,
        },
        {
          timestamp: ts(now, -12),
          message: `[${service}] Circuit breaker opened for database pool`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -10),
          message: `[${service}] 503 Service Unavailable returned for 23 requests`,
          level: 'error',
          service,
        },
        {
          timestamp: ts(now, -5),
          message: `[${service}] Circuit breaker half-open, testing connection`,
          level: 'info',
          service,
        },
      ],
    ...
    if (filter.includes('deploy') || filter.includes('change') || filter.includes('config')) {
      return [
        {
          timestamp: ts(now, -120),
          message: `[${service}] Deployment started: v2.3.1 → v2.4.0`,
          level: 'info',
          service,
        },
        {
          timestamp: ts(now, -118),
          message: `[${service}] Config change: WORKER_POOL_SIZE 10 → 5`,
          level: 'info',
          service,
        },
        {
          timestamp: ts(now, -115),
          message: `[${service}] Deployment completed: v2.4.0 (3 tasks rolled)`,
          level: 'info',
          service,
        },
        {
          timestamp: ts(now, -60),
          message: `[${service}] Feature flag toggled: new-cache-layer=true`,
          level: 'info',
          service,
        },
        {
          timestamp: ts(now, -10),
          message: `[${service}] Auto-scaling event: desired count 2 → 3`,
          level: 'warn',
          service,
        },
      ];
    }
    if (filter.includes('oom') || filter.includes('memory')) return scenarios['oom'];
    if (filter.includes('timeout') || filter.includes('connection')) return scenarios['timeout'];
  ...
  async queryMetrics(_creds: CloudCredentials, query: MetricQuery): Promise<MetricDataPoint[]> {
    ...
    const baseValue = metric.includes('cpu')
      ? 35
      : metric.includes('memory')
        ...
        : metric.includes('latency')
          ? 120
          : 50;
    const unit = metric.includes('cpu')
      ? 'Percent'
      : metric.includes('memory')
        ? 'Percent'
        : metric.includes('latency')
