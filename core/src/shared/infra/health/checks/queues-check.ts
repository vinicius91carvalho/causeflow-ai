...
export class QueuesHealthCheck {
    ...
    async check(): Promise<HealthCheckResult> {
        ...
        try {
            ...
            if (!client) {
                return { name: this.name, status: 'down', latencyMs: Date.now() - start, details: { error: 'Redis client not initialised' } };
            }
            await client.ping();
            return { name: this.name, status: 'ok', latencyMs: Date.now() - start, details: { transport: 'bullmq-on-redis', redisUrl: redactUrl(config.redis.url) } };
        } catch (err) {
            return {
                name: this.name,
                status: 'down',
                latencyMs: Date.now() - start,
                details: { transport: 'bullmq-on-redis', error: err instanceof Error ? err.message : 'Unknown' },
            };
        }
    }
}
