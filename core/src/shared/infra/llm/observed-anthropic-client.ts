...
export class ObservedAnthropicClient {
    ...
    metrics;
    traceContext;
    constructor(inner: LLMClient, tracer: Tracer, metrics: MetricRecorder, traceContext?: TraceContext) {
        this.inner = inner;
        this.tracer = tracer;
    ...
    async complete<T>(params: CompletionParams): Promise<CompletionResult<T>> {
        ...
        try {
            ...
            this.metrics.increment('llm.output_tokens', result.usage.outputTokens, { model: result.model });
