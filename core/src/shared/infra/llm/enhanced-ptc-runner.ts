...
function isCriticalToolFailure(err: unknown): boolean {
    ...
    if (err instanceof Error && err.message) {
        return /credentials?\s*(expired|invalid)/i.test(err.message) ||
        ...
        while (turns < maxTurns) {
            ...
            if (response.stop_reason !== 'tool_use') {
                ...
                if (minToolCalls && totalToolCalls < minToolCalls) {
                    ...
                    messages.push({
                        role: 'user' as const,
                        content: `You have only used ${totalToolCalls} tool${totalToolCalls === 1 ? '' : 's'} so far. ` +
            ...
            if (toolUseBlocks.length > 0) {
                ...
                for (const batch of batches) {
                    ...
                    if (batch.safe && batch.calls.length > 1) {
                        ...
                        batchResults = await this.executeBatchParallel(
                            batch.calls, agentConfig.toolHandler, maxConcurrency, agentConfig.signal,
                        );
                    } else {
                        // Serial execution
                        batchResults = await this.executeBatchSerial(
                            batch.calls, agentConfig.toolHandler, agentConfig.signal,
                        );
                    }

                    // Truncate per-tool results
                    for (const r of batchResults) {
                        const def = agentConfig.tools.find(t => t.name ===
                            toolUseBlocks.find(tb => tb.id === r.toolUseId)?.name);
                ...
                const toolResultBlocks: BetaToolResultBlockParam[] = allResults.map(r => {
                    ...
                    const budgetedContent = budgeted.find(b => b.toolUseId === r.toolUseId)?.content ?? r.content;
    ...
    private async executeBatchParallel(
        ...
        for (const call of calls) {
            ...
            if (batchAbort.signal.aborted) {
                results.push({ toolUseId: call.id, content: 'Aborted: sibling tool failed critically', isError: true });
                continue;
            }
