...
export class ObservedAgentRunner implements AgentRunner {
    ...
    metrics;
    traceContext;
    constructor(inner: AgentRunner, tracer: Tracer, metrics: MetricRecorder, traceContext?: TraceContext) {
        this.inner = inner;
        this.tracer = tracer;
    ...
    async run(agentConfig: AgentRunConfig): Promise<AgentRunResult> {
        ...
        try {
            ...
            if (result.totalUsage.cacheReadInputTokens) span.setAttribute('cacheReadTokens', result.totalUsage.cacheReadInputTokens);
            if (result.totalUsage.cacheCreationInputTokens) span.setAttribute('cacheCreationTokens', result.totalUsage.cacheCreationInputTokens);
            span.setOutput({
                response: result.response,
                toolCalls: result.toolCalls.map(tc => ({ name: tc.name, input: tc.input, output: tc.output.slice(0, 500) })),
                turns: result.turns,
            });
            span.setStatus('ok');
            // Core counters
            ...
            this.metrics.increment('agent.input_tokens', result.totalUsage.inputTokens, { model: result.model });
            this.metrics.increment('agent.output_tokens', result.totalUsage.outputTokens, { model: result.model });
            ...
            if (result.truncatedResults) {
                this.metrics.increment('agent.truncated_results', result.truncatedResults, { model: result.model });
            }
            if (result.parallelBatches) {
                this.metrics.increment('agent.parallel_batches', result.parallelBatches, { model: result.model });
            }
            ...
            if (result.totalUsage.cacheReadInputTokens) {
                this.metrics.increment('agent.cache_read_tokens', result.totalUsage.cacheReadInputTokens, { model: result.model });
            }
            if (result.totalUsage.cacheCreationInputTokens) {
                this.metrics.increment('agent.cache_creation_tokens', result.totalUsage.cacheCreationInputTokens, { model: result.model });
            }
            return result;
        }
        catch (err) {
