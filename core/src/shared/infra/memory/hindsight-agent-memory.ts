...
export class HindsightAgentMemory {
    ...
    async retain(tenantId: string, content: string, options?: RetainOptions): Promise<void> {
        ...
        try {
            ...
            getLogger().error({ event: 'hindsight.retain.failed', tenantId, durationMs: Date.now() - start, errorType: (err as Error).name }, 'hindsight.retain.failed');
    ...
    async recall(tenantId: string, query: string, options?: RecallOptions): Promise<MemoryEntry[]> {
        ...
        try {
            ...
            const result: MemoryEntry[] = response.results.slice(0, options?.maxResults ?? 5).map((r) => ({
                text: r.text,
                type: (r.type === 'world' || r.type === 'observation' ? r.type : 'world') as 'world' | 'observation',
                score: undefined,
            }));
            getLogger().info({ event: 'hindsight.recall', tenantId, durationMs: Date.now() - start, resultCount: result.length }, 'hindsight.recall');
            return result;
        }
        catch (err) {
            getLogger().error({ event: 'hindsight.recall.failed', tenantId, durationMs: Date.now() - start, errorType: (err as Error).name }, 'hindsight.recall.failed');
    ...
    async reflect(tenantId: string, query: string, options?: ReflectOptions): Promise<string> {
        ...
        try {
            ...
            getLogger().info({ event: 'hindsight.reflect', tenantId, durationMs: Date.now() - start }, 'hindsight.reflect');
            return response.text;
        }
        catch (err) {
            getLogger().error({ event: 'hindsight.reflect.failed', tenantId, durationMs: Date.now() - start, errorType: (err as Error).name }, 'hindsight.reflect.failed');
    ...
    async configureBank(tenantId: string, config: BankConfig): Promise<void> {
        ...
        try {
            ...
            getLogger().info({ event: 'hindsight.configureBank', tenantId, durationMs: Date.now() - start }, 'hindsight.configureBank');
            logger.info({ tenantId, bankId }, 'Hindsight bank configured');
        }
        catch (err) {
            getLogger().error({ event: 'hindsight.configureBank.failed', tenantId, durationMs: Date.now() - start, errorType: (err as Error).name }, 'hindsight.configureBank.failed');
