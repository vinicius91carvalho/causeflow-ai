...
import {
  createToolHandler,
  incidentDetailsTool,
} from '../modules/investigation/infra/investigation-tools.js';
...
import {
  publishProgress,
  INVESTIGATION_PROGRESS_CHANNEL,
} from '../shared/infra/pubsub/redis-pubsub.js';
...
async function workerBootstrap(ossMode?: boolean) {
  ...
  if (useOssRepos) {
    ...
    const { PgBillingAccountRepository } =
    ...
    const { DynamoIncidentRepository } =
      await import('../modules/ingestion/infra/dynamo-incident.repository.js');
    const { DynamoEvidenceRepository } =
      await import('../modules/triage/infra/dynamo-evidence.repository.js');
    const { DynamoToolCallRepository } =
      await import('../modules/triage/infra/dynamo-tool-call.repository.js');
    const { DynamoTenantRepository } =
      await import('../modules/tenant/infra/dynamo-tenant.repository.js');
    const { DynamoBillingAccountRepository } =
      await import('../modules/billing/infra/dynamo-billing-account.repository.js');
    const { DynamoSkillRepository } =
      await import('../modules/skills/infra/dynamo-skill.repository.js');
    const { DynamoHypothesisRepository } =
      await import('../modules/investigation/infra/dynamo-hypothesis.repository.js');
  ...
  const credentialVendor =
    config.isDev() || config.isTest()
      ? new StubCredentialVendor()
      : new (
          await import('../shared/infra/credentials/sts-credential-vendor.js')
        ).STSCredentialVendor(undefined, tenantRepo, getCloudIntegration);
  providerRegistry.registerCredentialVendor(credentialVendor);
  // Composio (conditional)
  const integrationToolProvider = config.composio.apiKey ? new ComposioToolProvider() : undefined;
  // Skills
  ...
  const investigateIncident = new InvestigateIncidentUseCase({
    ...
    messageQueue,
    remediationQueueUrl: useOssRepos
    ...
    agentMemory: new HindsightAgentMemory({
...
async function main() {
  ...
  if (relayUrl && relayToken) {
    ...
    investigationChannel.sendProgress({
      stage: 'provisioning',
      message: 'Worker connected — starting investigation...',
    });
  }
  // Watchdog timer — kill the worker if investigation takes too long
  const handleTimeout = () => {
    logger.error(
      { incidentId: INCIDENT_ID, tenantId: TENANT_ID },
      'Investigation timed out after 5 minutes',
    );
    incidentRepo
      .updateStatus(tid, iid, 'failed')
      .then(() => refundInvestigation.execute(tid))
      .then(() =>
        forwardProgress('investigation.failed', TENANT_ID!, {
          incidentId: INCIDENT_ID!,
          reason: 'timeout',
        }),
      )
  ...
  try {
    ...
    const memoryConfig = config.enhancedRunner.mastra
      ? {
          thread: {
            ...
            metadata: { incidentId: INCIDENT_ID, type: 'investigation' },
          },
          resource: TENANT_ID!,
        }
      : undefined;
    ...
    if (WORKER_MODE === 'followup') {
      ...
      logger.info(
        { incidentId: INCIDENT_ID, tenantId: TENANT_ID },
        'Followup worker — skipping investigation, entering idle mode',
      );
      ...
      if (completedIncident?.status === 'inconclusive') {
        ...
        logger.info(
          { incidentId: INCIDENT_ID, tenantId: TENANT_ID },
          'Investigation inconclusive — worker exiting',
        );
        process.exit(0);
      }
      ...
      if (!investigationChannel?.isConnected()) {
        ...
        logger.info(
          { incidentId: INCIDENT_ID, tenantId: TENANT_ID },
          'Investigation worker complete (no relay)',
        );
        process.exit(0);
      }
    }
    ...
    logger.info(
      ...
      'Investigation complete — entering idle mode',
    );
    ...
    activeChannel.sendComplete({
      ...
      recommendedActions: incident?.recommendedActions?.map(
        (a: { action: string; label?: string; description?: string; riskLevel?: string }) => ({
          action: a.action,
          label: a.label,
    ...
    try {
      if (credentialVendor) {
        ...
        if (vended.provider !== 'stub' && vended.credentials['accessKeyId']) {
          ...
          const { AWS_API_CALL_TOOL: loadedAwsApiTool } =
            await import('../modules/investigation/infra/aws-api-tool.js');
          followupTools.push(loadedAwsApiTool);
        }
      }
    } catch (err) {
      logger.warn(
        { err, incidentId: INCIDENT_ID },
        'Follow-up: failed to vend AWS credentials — continuing without AWS tools',
      );
    }
    try {
      if (integrationToolProvider) {
        ...
        if (connectedComposioApps.length > 0) {
          const composioTools = await integrationToolProvider.getTools(
            TENANT_ID!,
            connectedComposioApps,
          );
          ...
          logger.info(
            ...
            'Follow-up: Composio tools loaded',
          );
        }
      }
    } catch (err) {
    ...
    const baseFollowupHandler = createToolHandler({
      cloudCredentials: followupCredentials ?? {
        provider: 'stub',
        credentials: {},
    ...
    const followupToolHandler = async (
      name: string,
      ...
      if (name === 'report_finding') {
        ...
        logger.info(
          { incidentId: INCIDENT_ID, finding: finding.slice(0, 100), severity, category },
          'Follow-up: agent reported finding',
        );
      ...
      if (name === 'request_confirmation') {
        ...
        logger.info(
          { incidentId: INCIDENT_ID, question: question.slice(0, 100) },
          'Follow-up: request_confirmation (auto-default in Q&A)',
        );
      ...
      if (name === 'request_context') {
        ...
        logger.info(
          { incidentId: INCIDENT_ID, question: question.slice(0, 100) },
          'Follow-up: request_context (auto-default in Q&A)',
        );
    ...
    activeChannel.onShutdown?.((reason?: string) => {
      logger.info(
        { incidentId: INCIDENT_ID, reason },
        'Received shutdown signal from relay — exiting after grace',
      );
    ...
    activeChannel.onGuidance((guidance: string) => {
      ...
      logger.info(
        { incidentId: INCIDENT_ID, guidance: guidance.slice(0, 100) },
...
- Portuguese or English — match the operator's language.`,
    ...
    clearTimeout(watchdog);
    logger.fatal(
      { err, incidentId: INCIDENT_ID, tenantId: TENANT_ID },
      'Investigation worker failed',
    );
...
if (BULLMQ_CONSUMER_MODE) {
  ...
  main().catch((err) => {
    logger.fatal(
      { err, incidentId: INCIDENT_ID, tenantId: TENANT_ID },
      'Investigation worker unhandled error',
    );
    process.exit(1);
  });
}
