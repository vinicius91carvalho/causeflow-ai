...
export class PgNotificationRepository implements INotificationRepository {
  ...
  async findById(tenantId: TenantId, notificationId: NotificationId): Promise<Notification | null> {
    return store.get(key(tenantId, notificationId)) ?? null;
  }

  async findByTenant(tenantId: TenantId): Promise<Notification[]> {
  ...
  async countByTenant(tenantId: TenantId): Promise<number> {
    ...
    for (const k of store.keys()) {
      if (k.startsWith(tenantId + '::')) count++;
    }
    return count;
  }
}
