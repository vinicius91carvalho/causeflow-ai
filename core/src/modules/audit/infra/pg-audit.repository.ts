...
import { pgGet, pgInsert, pgDelete, pgQuery, pgQueryJson, pgUpdate } from '../../../shared/infra/db/postgres/pg-utils.js';
...
export class PgAuditRepository implements IAuditRepository {
    ...
    async findByTenant(tenantId: TenantId, options?: { limit?: number; cursor?: string; actorType?: 'user' | 'system' }): Promise<{ items: AuditEntry[]; cursor?: string }> {
    ...
    async findByAction(tenantId: TenantId, action: string, options?: { limit?: number; cursor?: string }): Promise<{ items: AuditEntry[]; cursor?: string }> {
        const rows = await pgQuery(TABLE, "tenant_id = $1 AND data->>'action' = $2", [String(tenantId), action], {
            orderBy: 'created_at DESC',
            limit: options?.limit ?? 50,
        });
        return { items: rows.map(toDomain) };
    ...
    async pseudonymizeActor(tenantId: TenantId, actorUserId: string, pseudonym: string): Promise<number> {
        const rows = await pgQueryJson(TABLE, "tenant_id = $1 AND data->>'actorUserId' = $2", [String(tenantId), actorUserId]);
    ...
    async findExpired(tenantId: TenantId, before: Date, options?: { limit?: number; cursor?: string }): Promise<{ items: AuditEntry[]; cursor?: string }> {
        const rows = await pgQuery(TABLE, 'tenant_id = $1 AND created_at < $2', [String(tenantId), before.toISOString()], {
