...
function toDomain(row: any): Incident {
    return {
        ...
        shadowInvestigationMode: row.data['shadowInvestigationMode'] as Incident['shadowInvestigationMode'],
...
export class PgIncidentRepository implements IIncidentRepository {
    ...
    async update(tenantId: TenantId, incidentId: IncidentId, data: Partial<Omit<Incident, 'incidentId' | 'tenantId' | 'createdAt'>>): Promise<Incident> {
    ...
    async updateStatus(tenantId: TenantId, incidentId: IncidentId, status: IncidentStatus, resolvedAt?: string): Promise<Incident> {
    ...
    async listByTenant(tenantId: TenantId, options?: ListOptions): Promise<{ items: Incident[]; cursor?: string }> {
        ...
        return {
            ...
            cursor: rows.length === (options?.limit ?? 20) ? String((options?.cursor ? parseInt(options.cursor, 10) : 0) + rows.length) : undefined,
        };
    }

    async findBySeverity(tenantId: TenantId, severity: Severity, options?: ListOptions): Promise<{ items: Incident[]; cursor?: string }> {
        const rows = await pgQuery(TABLE, "tenant_id = $1 AND data->>'severity' = $2", [tenantId, severity], {
            orderBy: 'created_at DESC',
            limit: options?.limit ?? 20,
        });
    ...
    async findByStatus(tenantId: TenantId, severity: Severity, status: IncidentStatus, options?: ListOptions): Promise<{ items: Incident[]; cursor?: string }> {
        const rows = await pgQuery(TABLE, "tenant_id = $1 AND data->>'severity' = $2 AND data->>'status' = $3", [tenantId, severity, status], {
            orderBy: 'created_at DESC',
            limit: options?.limit ?? 20,
        });
        return { items: rows.map(toDomain) };
    ...
    async listByCreatedAt(tenantId: TenantId, options?: ListOptions & { order?: 'asc' | 'desc' }): Promise<{ items: Incident[]; cursor?: string }> {
        const rows = await pgQuery(TABLE, 'tenant_id = $1', [tenantId], {
