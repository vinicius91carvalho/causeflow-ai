...
const updateStatusSchema = z.object({
    status: z.enum(['open', 'triaging', 'investigating', 'awaiting_approval', 'remediating', 'resolved', 'closed']),
});
...
const VALID_STATUSES = ['open', 'triaging', 'investigating', 'awaiting_approval', 'remediating', 'resolved', 'closed', 'aborted', 'failed', 'inconclusive'];
export function createIncidentRoutes(useCases: IncidentUseCases): Hono<AppEnv, import("hono/types").BlankSchema, "/"> {
    ...
    app.get('/', async (c) => {
        ...
        if (severity && VALID_SEVERITIES.includes(severity)) {
            if (status && VALID_STATUSES.includes(status)) {
                const result = await useCases.incidentRepo.findByStatus(tenantId, severity as any, status as any, { limit, cursor });
                return c.json(sanitizeList(result));
            }
            const result = await useCases.incidentRepo.findBySeverity(tenantId, severity as any, { limit, cursor });
            return c.json(sanitizeList(result));
        }
        {
            const order = sort === 'asc' ? 'asc' : 'desc';
            const result = await useCases.incidentRepo.listByCreatedAt(tenantId, { limit, cursor, order });
            return c.json(sanitizeList(result));
        }
    });
