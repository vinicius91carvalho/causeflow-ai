...
} from '../domain/triage.prompts.js';
import { logger } from '../../../shared/infra/logger.js';
import { config } from '../../../shared/config/index.js';
import {
  isLocalLlmFailClosedMode,
...
export class TriageIncidentUseCase {
  ...
  constructor(
    depsOrRepo: TriageIncidentDeps | IIncidentRepository,
    evidenceRepo?: IEvidenceRepository,
    eventBus?: IEventBus,
    llmClient?: LLMClient,
    messageQueue?: MessageQueue,
  ...
  async execute(tenantId: TenantId, incidentId: IncidentId): Promise<TriageResult> {
    const span = this.tracer.startSpan(
      ...
      { userId: String(tenantId), sessionId: String(incidentId) },
      'span',
    );
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          ...
          'LLM triage failed — using fallback default',
        );
        result = {
          priority: 'high',
          summary:
            'Triage completed via fallback (LLM unavailable). Assigned default high severity for local investigation.',
          suggestedAgents: [
            'log_analyst',
            'metric_analyst',
            'change_detector',
      ...
      if (
        ...
        const merged = Array.from(new Set([...result.suggestedAgents, ...foundationalAgents]));
      ...
      try {
        ...
        logger.warn(
          {
            incidentId,
            err: evidenceErr instanceof Error ? evidenceErr.message : String(evidenceErr),
          },
          'Failed to persist triage evidence — non-critical',
        );
      }
  ...
  async buildSmartPrompt(tenantId: TenantId): Promise<string> {
    ...
    if (this.hasAwsCredentials) connectedIntegrations.push('AWS');
    if (this.hasGitHub) connectedIntegrations.push('GitHub');
    if (this.hasRelay) connectedIntegrations.push('Relay');
    ...
    if (this.integrationToolProvider) {
      try {
        ...
        logger.warn(
          { err, tenantId },
          'Failed to fetch integration status for triage — using defaults',
        );
      }
    }
