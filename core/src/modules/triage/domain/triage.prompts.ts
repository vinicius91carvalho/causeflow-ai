...
export const triageResultSchema = z.object({
    ...
    category: z.enum(['infrastructure', 'application', 'deployment', 'third_party', 'database', 'unknown']),
...
const AGENT_CATALOG = [
    ...
    { name: 'change_detector', description: 'Finds recent deployments and config changes', requires: 'AWS' },
    { name: 'code_analyzer', description: 'Analyzes git history and code changes', requires: 'GitHub' },
    { name: 'db_analyst', description: 'Queries customer databases', requires: 'Relay' },
    { name: 'issue_correlator', description: 'Searches related issues/tickets', requires: 'Jira/Linear/ClickUp via Composio' },
    { name: 'apm_analyst', description: 'Analyzes APM traces, errors, performance', requires: 'Datadog/Sentry/New Relic via Composio' },
    { name: 'notification_sender', description: 'Posts investigation updates', requires: 'Slack/Teams/Discord via Composio' },
];
/**
...
function resolveAvailableRequirements(connectedIntegrations: string[]) {
    ...
    for (const integration of connectedIntegrations) {
        const reqs = (INTEGRATION_TO_REQUIREMENT as Record<string, string[]>)[integration.toLowerCase()];
        if (reqs) {
            for (const r of reqs)
                requirements.add(r);
        }
...
export function buildTriagePrompt(connectedIntegrations: string[]): string {
    ...
    if (hasIntegrationInfo) {
        ...
        const agentLines = AGENT_CATALOG.map((a) => `- ${a.name}: ${a.description} (requires: ${a.requires})${availableAgents.includes(a) ? '' : ' [NOT AVAILABLE]'}`).join('\n');
