...
export class StripePlanCatalogService implements IPlanCatalogService {
    ...
    async getPlanByPriceId(priceId: string): Promise<PlanDefinition | null> {
        ...
        return catalog.plans.find(
            (p) => p.stripePriceId === priceId || p.metered?.invPriceId === priceId || p.metered?.evtPriceId === priceId,
        ) ?? null;
    }
    ...
    private async fetchFromStripe(): Promise<CatalogCache> {
        ...
        for (const price of prices.data) {
            ...
            if (!planMap.has(planKey)) {
                const features = meta['features'] ? meta['features'].split('|').map((f: string) => f.trim()) : [];
                planMap.set(planKey, {
                    planKey,
                    productId,
                    productName,
        ...
        for (const entry of planMap.values()) {
            plans.push({
                ...
                metered: entry.invPriceId && entry.evtPriceId
    ...
    private buildFallbackCatalog(): CatalogCache {
        const plans: PlanDefinition[] = (Object.entries(PLAN_QUOTAS) as [TenantPlan, typeof PLAN_QUOTAS[TenantPlan]][]).map(
            ([key, quota]) => ({
                planKey: key,
