...
export class RecordUsageUseCase {
    ...
    async execute(input: RecordUsageInput): Promise<UsageRecord> {
        ...
        const incremented = await this.billingAccountRepo.incrementUsageAtomic(input.tenantId, input.type);
        ...
        if (this.meterEventService && this.tenantRepo) {
            ...
            if (tenant?.stripeCustomerId) {
                const eventName = input.type === 'investigation'
                    ? 'causeflow_investigation'
                    : 'causeflow_alert_ingested';
