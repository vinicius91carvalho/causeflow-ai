...
import type { Hypothesis, HypothesisEvidenceRef, HypothesisStatus } from '../domain/hypothesis.entity.js';
...
export class DynamoHypothesisRepository implements IHypothesisRepository {
    ...
    async listByIncident(tenantId: TenantId, incidentId: IncidentId): Promise<Hypothesis[]> {
        const result = await HypothesisEntity.query
            .primary({ tenantId, incidentId })
            .go();
