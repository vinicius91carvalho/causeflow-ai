...
export const awsApiCallInputSchema = z.object({
  service: z
    .string()
    .describe('AWS service short name (e.g., "ecs", "rds", "elbv2", "dynamodb", "s3")'),
  action: z
    .string()
    .describe(
      'AWS API action name (e.g., "DescribeServices", "DescribeDBInstances", "DescribeTargetHealth"). Must be read-only (Describe/Get/List/Query).',
    ),
  params: z
    .record(z.unknown())
    .optional()
    .default({})
    .describe(
      'API call parameters as a JSON object (e.g., {"cluster": "prod", "services": ["api"]})',
    ),
  region: z
    .string()
    .optional()
    .describe(
      'AWS region override (e.g., "us-east-1"). Defaults to the integration\'s region. Use when a resource lives in a different region than the integration default — DynamoDB tables, S3 buckets, CloudWatch logs, etc. are region-scoped.',
    ),
});
...
export function createAwsApiToolHandler(
  credentials: CloudCredentials,
): (name: string, input: Record<string, unknown>) => Promise<string | null> {
  return async (name, input) => {
    if (name !== 'aws_api_call') return null;
    ...
    if (mod[commandName] == null) {
      throw new Error(
        `Unknown action "${validated.action}" for service "${validated.service}". Command "${commandName}" not found in SDK package.`,
      );
    }
