...
import type { AgentRunner, ToolCallRecord, ToolDefinition } from '../../../../../shared/application/ports/agent-runner.port.js';
...
export class HypothesisMode implements InvestigationMode {
    ...
    async run(input: InvestigationInput): Promise<InvestigationResult> {
        ...
        // ── Phase 2: VALIDATOR ─────────────────────────────────────────
        channel?.sendProgress({ stage: 'validate', message: `Validating ${hypotheses.length} hypotheses with tool calls…` });
        await this.deps.eventBus.publish({
            eventType: 'investigation.progress',
            occurredAt: new Date().toISOString(),
            tenantId,
            payload: { incidentId, stage: 'validate', message: `Validator gathering evidence for ${hypotheses.length} hypotheses` },
        });
...
${hypotheses
    .map((h, i) => `H${i + 1} (id=${h.hypothesisId}, prior=${(h.confidence * 100).toFixed(0)}%): ${h.statement}
...
Gather evidence for/against each hypothesis. For every tool call, state in ONE sentence which hypothesis it is testing and what outcome supports/contradicts it. End with a summary listing, per hypothesis, the strongest supporting and contradicting evidence found.`;
        ...
        // parity is preserved).
        await this.persistValidatorEvidence(incident.tenantId, incident.incidentId, validatorRun.toolCalls, validatorRun.response);
        ...
        if (allExhausted && !this.deps.disableReseek) {
            ...
            const rejectedInfo = hypotheses.map((h) => {
                const ruling = firstJudgeResult.output.rulings.find((r) => r.hypothesisId === h.hypothesisId);
...
${validatorRun.toolCalls
    .map((c, i) => `${i + 1}. ${c.name}(${JSON.stringify(c.input).slice(0, 120)}) → ${(c.output ?? '').slice(0, 300)}`)
    .join('\n')}`;
        ...
        for (const r of ruling.rulings) {
            ...
            if (!h) {
                logger.warn({ unknownHypothesisId: r.hypothesisId, incidentId }, 'Judge referenced unknown hypothesis id — skipping update');
                continue;
            }
