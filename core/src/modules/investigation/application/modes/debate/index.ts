...
import type { InvestigationInput, InvestigationResult, StructuredAction } from '../../../domain/investigation.types.js';
...
import type { AgentRunner, AgentRunResult, ToolDefinition } from '../../../../../shared/application/ports/agent-runner.port.js';
...
export class DebateMode implements InvestigationMode {
    ...
    async run(input: InvestigationInput): Promise<InvestigationResult> {
        ...
        // ── Phase 1: SEEKER ────────────────────────────────────────────
        await this.emitProgress(tenantId, incidentId, 'hypothesize', 'Generating candidate hypotheses…');
        channel?.sendProgress({ stage: 'hypothesize', message: 'Generating candidate hypotheses…' });

        const seekerModel = this.deps.seekerColdStartModel && this.deps.seekerModel
        ...
        await this.emitProgress(tenantId, incidentId, 'advocate', `Running ${hypotheses.length} advocate agents in parallel…`);
        channel?.sendProgress({ stage: 'advocate', message: `Running ${hypotheses.length} advocate agents…` });

        const advocate = new Advocate({ agentRunner: this.deps.agentRunner, model: this.deps.advocateModel });
        ...
        await this.emitProgress(tenantId, incidentId, 'prosecute', `Running ${hypotheses.length} prosecutor agents in parallel…`);
        channel?.sendProgress({ stage: 'prosecute', message: `Running ${hypotheses.length} prosecutor agents…` });

        const prosecutor = new Prosecutor({ agentRunner: this.deps.agentRunner, model: this.deps.prosecutorModel });
        ...
        // ── Phase 4: JUDGE ──────────────────────────────────────────────
        await this.emitProgress(tenantId, incidentId, 'judge', 'Scoring hypotheses and selecting winner…');
        ...
        if (allExhausted && !this.deps.disableReseek) {
            ...
            const observationsBlock = rounds
                .map((round, i) => `### Hypothesis ${i + 1} (refuted)
...
Prosecutor summary: ${round.prosecutor.response}`)
                    ...
                    advocate.run({
                        ...
                        memory: {
                            thread: {
                                ...
                                metadata: { incidentId, role: 'advocate', hypothesisId: h.hypothesisId, reseek: true },
                            },
                            resource: tenantId,
                        },
                    }),
                    ...
                    prosecutor.run({
                        ...
                        memory: {
                            thread: {
                                ...
                                metadata: { incidentId, role: 'prosecutor', hypothesisId: h.hypothesisId, reseek: true },
                            },
                            resource: tenantId,
                        },
                    }),
            ...
            if (reseekTop > originalTop) {
                logger.info({ incidentId, originalTop, reseekTop }, 'Re-seek produced a stronger ruling — using it');
                ...
                logger.info({ incidentId, originalTop, reseekTop }, 'Re-seek did not beat original — keeping original');
            }
        }
        ...
        for (const r of ruling.rulings) {
            ...
            if (!h) {
                logger.warn({ unknownHypothesisId: r.hypothesisId, incidentId }, 'Judge referenced unknown hypothesis id — skipping');
                continue;
            }
        ...
        this.deps.metrics?.gauge('investigation.hypotheses_count', hypothesesForPersist.length, MODE_TAG);
        const winnerRuling = ruling.rulings.find((r) => r.status === 'confirmed');
        if (winnerRuling) {
            this.deps.metrics?.histogram(
                'investigation.winner_score',
                winnerRuling.finalScore,
                MODE_TAG,
            );
        }
