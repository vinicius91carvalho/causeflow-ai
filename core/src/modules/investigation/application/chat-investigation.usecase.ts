...
export class ChatInvestigationUseCase {
  ...
  async execute(input: ChatInvestigationInput): Promise<ChatInvestigationOutput> {
    ...
    await this.mirrorToChatPlatform(
      tenantId,
      incident,
      `*Operator:* ${message}`,
  ...
  private async handleCorrectionOrContext(
    ...
    if (this.deps.agentMemory) {
      ...
      await this.deps.agentMemory
        .retain(
          tenantId,
          `[${tag.toUpperCase()}] Operator feedback on incident "${incident.title}": ${message}` +
            (isCorrection && incident.rootCause
              ? ` (Original root cause was: "${incident.rootCause}")`
              : ''),
          {
    ...
    if (isCorrection && reinvestigationTriggered) {
      response =
        `Got it — I've recorded your correction and started a new investigation with this context. ` +
      ...
      response =
        `I've recorded your correction. There's already an investigation running — ` +
        `the context has been added and will be available to the current investigation. ` +
        `No need to start a new one.`;
    } else if (isCorrection) {
      response =
        `I've recorded your correction for future investigations. ` +
      ...
      response =
        `Thanks for the additional context. I've saved it as evidence on this incident. ` +
    ...
    logger.info(
      { tenantId, incidentId, intent, reinvestigationTriggered },
      'Chat correction/context handled',
    );
  ...
  private async handleQuestion(
    ...
    const evidenceSummary = evidence
      .slice(0, 5)
      .map((e) => `[${e.agentRole ?? 'agent'}] ${e.content?.slice(0, 300) ?? ''}`)
      .join('\n');
    ...
    if (this.deps.chatHistory) {
      ...
      if (history.length > 0) {
        historyContext =
          ...
          history
            .slice(-10)
            .map((m) => `${m.role === 'user' ? 'Operator' : 'Agent'}: ${m.content.slice(0, 300)}`)
            .join('\n');
      }
    }
    ...
    if (this.deps.agentMemory) {
      try {
        const memories = await this.deps.agentMemory.recall(
          tenantId,
...
${evidenceSummary || 'No evidence available.'}${memoryContext}${historyContext}`;
    ...
    try {
      const result = await this.deps.agentRunner.run({
        ...
        memory: {
          thread: {
            ...
            metadata: { incidentId: incidentId as string, type: 'investigation' },
          },
      ...
      logger.warn(
        { err, tenantId, incidentId },
        'Follow-up agent unavailable — using investigation-context stub reply',
      );
    ...
    if (this.deps.dispatchFollowupWorker) {
      this.deps.dispatchFollowupWorker(incidentId as string, tenantId as string).catch((err) => {
        logger.warn(
          { err, incidentId },
          'Failed to dispatch followup worker — REST fallback will continue',
        );
      });
    }
  ...
  private buildStubFollowupReply(
    ...
    const evidenceHint = evidenceSummary ? ` Key evidence: ${evidenceSummary.slice(0, 400)}` : '';
    return (
      `Based on the investigation "${incident.title}", the identified root cause is: ${rootCause}. ` +
  ...
  private async resolveChatPlatform(tenantId: TenantId): Promise<ChatPlatform | undefined> {
    ...
    if (
      tenant?.settings?.chatProvider === 'slack' &&
      ...
      try {
        ...
        const { SlackChatPlatform } =
          await import('../../../shared/infra/chat/slack-chat-platform.js');
        ...
        logger.warn(
          { err, tenantId },
          'Failed to resolve Slack chat platform — falling back to web portal',
        );
      }
    }
  ...
  async getHistory(
    tenantId: TenantId,
    incidentId: IncidentId,
  ): Promise<Array<{ role: string; content: string; createdAt: string; slackThreadId?: string }>> {
