...
import {
  ...
  IncidentNotInvestigatableError,
} from '../domain/investigation.errors.js';
import {
  CircuitBreakerOpenError,
  type CircuitState,
} from '../../../shared/infra/llm/circuit-breaker.js';
import { isLocalLlmFailClosedMode } from '../../../shared/infra/llm/llm-factory.js';
import {
  assertLocalLlmReachable,
  LOCAL_LLM_UNAVAILABLE_MESSAGE,
} from '../../../shared/infra/llm/local-llm-guard.js';
...
} from './intelligence/investigation-memory.js';
...
import type {
  CloudProvider,
  CloudCredentials,
} from '../../../shared/application/ports/cloud-provider.port.js';
...
function buildAgentSummary(
  ...
  const agentResponse =
    response.length > MAX_RESPONSE ? response.slice(0, MAX_RESPONSE) + '...' : response;

  // Key tool call results (filter out memory tools, include data-bearing tools)
  const dataTools = toolCalls.filter(
    (tc) =>
      ![
        'remember_finding',
      ...
      ].includes(tc.name) && !tc.output.startsWith('Tool `'), // exclude failed tools
  );
  ...
  const toolResults = dataTools
    .map((tc) => {
      const output =
        tc.output.length > MAX_TOOL_OUTPUT
...
function formatWaveFindings(results: SubAgentResult[]): string {
  ...
  const sections = results.map(
    (r: SubAgentResult) =>
      `### ${r.agentRole} (confidence: ${(r.confidence * 100).toFixed(0)}%)\n${r.findings.join('\n')}`,
  );
...
function calculateConfidence(result: AgentRunResult): number {
  ...
  if (result.toolCalls.length > 0) score += 0.1;
  if (result.toolCalls.length >= 3) score += 0.1;
  ...
  const successRate =
    result.toolCalls.length > 0 ? successfulCalls.length / result.toolCalls.length : 0;
  score += successRate * 0.2; // 0 to 0.2 based on success rate
  // Response quality
  if (result.response.length > 300) score += 0.05;
  if (result.response.match(/root cause|caused by|identified|confirmed/i)) score += 0.1;
  if (!result.response.match(/unable to|could not|failed to|no data/i)) score += 0.05;
...
export class InvestigateIncidentUseCase {
  ...
  private failClosedIfLocalLlmMode(
    incidentId: IncidentId,
    reason = LOCAL_LLM_UNAVAILABLE_MESSAGE,
  ): void {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const details = invalidRefs
          .map(
            (r) =>
              `  - Finding "${r.finding.slice(0, 80)}" cites unknown ids: ${r.badIds.join(', ')}`,
          )
          .join('\n');
        logger.warn(
          {
  ...
  private async logSyntheticMemoryCall(params: {
    ...
    const output =
      params.output.length > MAX_OUTPUT_BYTES
    ...
    try {
      ...
      logger.warn(
        { err, incidentId: params.incidentId, name: params.name },
        'Failed to persist synthetic memory tool call',
      );
      return null;
    }
  }
  ...
  async buildTenantCapabilities(tenantId: TenantId): Promise<TenantCapabilities> {
    ...
    if (this.integrationToolProvider) {
      try {
        ...
        composioApps = connections.filter((c) => c.status === 'connected').map((c) => c.provider);
      } catch {
        /* degrade gracefully */
      }
    }
  ...
  buildCapabilitiesPrompt(capabilities: TenantCapabilities, composioApps: string[]): string {
    ...
    if (capabilities.hasAws) {
      lines.push(
        '- **AWS** via `aws_api_call` — CloudWatch logs/metrics, ECS/EC2/RDS/Route53/CloudFront/Lambda (read-only). Primary source for infrastructure state. See the main methodology above.',
      );
    }
    ...
    const appGuidance: Record<string, string> = {
      github:
        '- **GitHub** via `composio_GITHUB_*` — recent PRs, commits, issues, releases. Trigger: incident started after a deploy, or you suspect a code change. Start with `composio_GITHUB_PULLS_LIST` (state=closed, sort=updated) for merges in the last 24-48h on the affected service repo.',
      gitlab:
        '- **GitLab** via `composio_GITLAB_*` — merge requests, commits, pipelines. Trigger: same as GitHub — correlate recent merges with incident timeline.',
      bitbucket:
        '- **Bitbucket** via `composio_BITBUCKET_*` — PRs, commits, repositories. Trigger: check recent PRs on the affected service repo.',
      slack:
        '- **Slack** via `composio_SLACK_*` — messages, channels, threads. Trigger: ALWAYS before concluding — search #incidents, #engineering, #oncall for ongoing discussion about the same symptoms. Saves duplicate investigation work.',
      teams:
        '- **Microsoft Teams** via `composio_TEAMS_*` — messages, channels. Trigger: same as Slack — search for ongoing discussion before concluding.',
      discord:
        '- **Discord** via `composio_DISCORD_*` — messages, channels. Trigger: search for ongoing discussion before concluding.',
      ...
      linear:
        '- **Linear** via `composio_LINEAR_*` — issues, projects, cycles. Trigger: same as Jira — look for open/recently-closed issues on the affected component.',
      trello:
        '- **Trello** via `composio_TRELLO_*` — boards, cards. Trigger: search cards for the affected service in active lists.',
      shortcut:
        '- **Shortcut** via `composio_SHORTCUT_*` — stories, iterations, epics. Trigger: check current iteration for stories touching the affected service; recently-completed stories may have introduced the change.',
      clickup:
        '- **ClickUp** via `composio_CLICKUP_*` — tasks, spaces. Trigger: search tasks for the affected service.',
      asana:
        '- **Asana** via `composio_ASANA_*` — tasks, projects. Trigger: search tasks for the affected service.',
      notion:
        "- **Notion** via `composio_NOTION_*` — runbooks, architecture docs, ADRs, postmortems. Trigger: symptom you don't recognize, service behavior that seems intentional but unclear, or before recommending risky remediation — check for a documented runbook or prior postmortem.",
      confluence:
        '- **Confluence** via `composio_CONFLUENCE_*` — runbooks, architecture docs. Trigger: same as Notion — check for documented runbook before concluding.',
      datadog:
        '- **Datadog** via `composio_DATADOG_*` — monitors, metrics, logs. Trigger: use instead of/in addition to CloudWatch when tenant uses Datadog as primary observability.',
      sentry:
        '- **Sentry** via `composio_SENTRY_*` — errors, issues, releases. Trigger: if incident is application-level (5xx, exceptions), check Sentry for the spike — group by release to correlate with deploys.',
      pagerduty:
        "- **PagerDuty** via `composio_PAGERDUTY_*` — incidents, services, schedules. Trigger: check if a related PagerDuty incident is already open (avoid duplicate investigation) and who's on-call for the affected service.",
      newrelic:
        '- **New Relic** via `composio_NEWRELIC_*` — APM, alerts, dashboards. Trigger: APM transactions, dependency maps when tenant uses New Relic.',
      hubspot:
        '- **HubSpot** via `composio_HUBSPOT_*` — customer context. Trigger: when incident has customer-facing impact and you need to understand the account affected.',
      zendesk:
        '- **Zendesk** via `composio_ZENDESK_*` — support tickets. Trigger: check for customer tickets reporting the same symptom — confirms scope and customer impact.',
      intercom:
        '- **Intercom** via `composio_INTERCOM_*` — customer conversations. Trigger: same as Zendesk — confirm customer-facing impact.',
    };

    for (const app of composioApps) {
      lines.push(
        appGuidance[app] ??
          `- **${app}** via \`composio_${app.toUpperCase()}_*\` — connected integration, use when relevant to the incident.`,
      );
    }

    if (capabilities.hasRelay) {
      lines.push(
        '- **Database (live)** via `db_*` tools — PostgreSQL/MongoDB read-only through secure relay. Trigger: you need to verify actual data state (row counts, recent inserts, schema), not just logs about data.',
      );
    }
  ...
  async execute(input: InvestigationInput): Promise<InvestigationResult | void> {
    ...
    input.channel?.sendProgress({
      stage: 'memory_check',
      message: 'Checking knowledge from past incidents...',
    });
    ...
    if (this.agentMemory && !hasOperatorCorrection) {
      try {
        ...
        const knownSolutionCheck = await this.agentMemory.reflect(tenantId, reflectPrompt, {
          budget: 'mid',
          tags: ['investigation', 'remediation'],
        });
          ...
          if (jsonMatch) {
            ...
            if (parsed.known && parsed.rootCause && (parsed.confidence ?? 0) >= 0.85) {
              ...
              if (supportingMemories.length < MIN_SUPPORTING_MEMORIES) {
                logger.info(
                  {
                    ...
                    required: MIN_SUPPORTING_MEMORIES,
                  },
                  'Known solution claim not backed by enough memories — proceeding with full investigation',
                );
                input.channel?.sendProgress({
                  stage: 'memory_check',
                  message: `Memory suggested a known solution but only ${supportingMemories.length} supporting memory found — investigating to confirm...`,
                });
              } else {
                logger.info(
                  {
                    incidentId,
                    confidence: parsed.confidence,
                    supportingMemories: supportingMemories.length,
                  },
                  'Known solution found via memory — skipping investigation',
                );
                ...
                const knownQuote =
                  knownSolutionCheck && knownSolutionCheck.length > 0
        ...
        /* non-critical — proceed with full investigation */
      }
    }
    ...
    if (config.enhancedRunner.orchestratorMode) {
      try {
        ...
        logger.warn(
          { err, incidentId, tenantId },
          'Orchestrator execution failed — producing stub result',
        );
    ...
    if (this.integrationToolProvider) {
      try {
        const connections = await this.integrationToolProvider.getConnectionStatus(tenantId);
        connectedComposioApps = connections
          .filter((c) => c.status === 'connected')
          .map((c) => c.provider);
        composioTools =
          connectedComposioApps.length > 0
            ? await this.integrationToolProvider.getTools(tenantId, connectedComposioApps)
            : [];
        if (composioTools.length > 0) {
          logger.info(
            { incidentId, composioToolCount: composioTools.length, connectedComposioApps },
            'Composio tools loaded for investigation',
          );
        }
      } catch (err) {
    ...
    logger.info(
      {
        tenantId,
        capabilities: {
          hasAws: capabilities.hasAws,
          composioApps: capabilities.composioApps,
          hasRelay: capabilities.hasRelay,
        },
      },
      'Tenant capabilities resolved',
    );
    ...
    if (config.enhancedRunner.scoutAgent) {
      try {
        ...
        const { successful } = await this.runAgentWave(
          ['scout'],
          incident,
          tenantId,
          incidentId,
        ...
        await this.eventBus.publish({
          ...
          payload: {
            ...
            wave: 0,
            message: 'Wave 0 completed: scout',
          },
        });
      } catch (err) {
    ...
    if (wave1Roles.length > 0) {
      await this.eventBus.publish({
        ...
        payload: {
          ...
          stage: 'wave_started',
          wave: 1,
          message: `Wave 1 started: ${wave1Roles.join(', ')}`,
        },
      });
      const { successful, failed } = await this.runAgentWave(
        wave1Roles,
        incident,
        tenantId,
        incidentId,
      ...
      await this.eventBus.publish({
        ...
        payload: {
          ...
          stage: 'wave_completed',
          wave: 1,
          message: `Wave 1 completed: ${successful.length} succeeded, ${failed.length} failed`,
        },
      });
    }
      ...
      try {
        ...
        if (selectedSkills.length > 0) {
          logger.info(
            ...
            'Tenant skills selected for investigation',
          );
        }
      } catch (err) {
        logger.warn(
          { err, incidentId },
          'Skill selection failed — continuing without tenant skills',
        );
      }
    }
    ...
    if (wave2Roles.length > 0) {
      ...
      await this.eventBus.publish({
        ...
        payload: {
          ...
          stage: 'wave_started',
          wave: 2,
          message: `Wave 2 started: ${wave2Roles.join(', ')}`,
        },
      });
      const { successful, failed } = await this.runAgentWave(
        wave2Roles,
      ...
      await this.eventBus.publish({
        ...
        payload: {
          ...
          stage: 'wave_completed',
          wave: 2,
          message: `Wave 2 completed: ${successful.length} succeeded, ${failed.length} failed`,
        },
      });
    }
    ...
    if (successfulResults.length === 0) {
      ...
      this.failClosedIfLocalLlmMode(incidentId);
      logger.warn(
        { incidentId, tenantId, failedAgents },
        'All sub-agents failed — completing with stub result',
      );
      const stubRootCause = 'Unable to determine root cause (LLM service unavailable)';
      const investigationDurationMs = Date.now() - investigationStartMs;
      const stubAgentRoles = failedAgents.length > 0 ? failedAgents.map((a) => a.role) : validRoles;
...
${findingsSummary}`;
    ...
    try {
      ...
      investigationResult = {
        ...
        potentialRootCause:
          allFindings[0] ?? 'Unable to synthesize — see individual agent findings',
        recommendedActions: [],
      ...
      try {
        await this.eventBus.publish({
          ...
          payload: {
            ...
            stage: 'wave_started',
            wave: 3,
            message: 'Wave 3 started: diagnosis_verifier',
          },
        });
        ...
        if (verifyResults.length > 0) {
          ...
          investigationResult.findings.push({
            text: `[Verification] ${verifyResult.findings.join('; ')}`,
            evidenceIds: [],
          });
          await this.evidenceRepo.create({
            tenantId,
        ...
        await this.eventBus.publish({
          ...
          payload: {
            ...
            stage: 'wave_completed',
            wave: 3,
            message: 'Wave 3 completed: diagnosis_verifier',
          },
        });
      } catch (err) {
        logger.warn(
          { err, incidentId },
          'Verification agent failed — continuing without verification',
        );
      }
    }
    // Fallback: Hindsight memory falsification (when verification agent is disabled)
      ...
      try {
        const challenge = await this.agentMemory.reflect(
          tenantId,
        ...
        /* non-critical — investigation proceeds without falsification */
      }
    }
    ...
    const wave0CostUsd = successfulResults
      .filter((r) => r.wave === 0)
      .reduce((sum, r) => sum + r.costUsd, 0);
    const wave1CostUsd = successfulResults
      .filter((r) => r.wave === 1)
      .reduce((sum, r) => sum + r.costUsd, 0);
    const wave2CostUsd = successfulResults
      .filter((r) => r.wave === 2)
      .reduce((sum, r) => sum + r.costUsd, 0);
    const wave3CostUsd = successfulResults
      .filter((r) => r.wave === 3)
      .reduce((sum, r) => sum + r.costUsd, 0);
    ...
    if (this.agentMemory) {
      retainInvestigationSummary(
        ...
        incidentId,
        incident.title,
        investigationResult.potentialRootCause,
        investigationResult.findings,
        investigationResult.recommendedActions,
        totalCostUsd,
        investigationDurationMs,
        successfulResults.map((r) => r.agentRole),
      ).catch(() => {
        /* non-critical */
      });
    }
    ...
    const nextStatus =
      investigationResult.recommendedActions.length > 0 ? 'awaiting_approval' : 'resolved';
    ...
    if (
      this.messageQueue &&
      this.remediationQueueUrl &&
      investigationResult.recommendedActions.length > 0
    ) {
  ...
  private async executeOrchestrator(
    ...
    input.channel?.sendProgress({
      ...
      message: 'Detecting AWS, integrations, and relay capabilities...',
    });
    let hasAws = false;
    if (this.credentialVendor) {
      try {
        ...
        logger.warn(
          { err: err instanceof Error ? err.message : err, tenantId, incidentId },
          'AWS credential vend failed during capability check',
        );
        hasAws = false;
      }
    }
    ...
    logger.info(
      {
        ...
        incidentId,
        hasAws,
        composioApps: connectedComposioApps.length,
        hasRelay: capabilities.hasRelay,
      },
    ...
    if (this.credentialVendor) {
      try {
        ...
        logger.warn(
          { tenantId, err: vendErr instanceof Error ? vendErr.message : String(vendErr) },
          'Credential vend failed — using stub',
        );
    ...
    const serviceInfo =
      ...
      incident.sourceProvider !== 'chat' &&
      incident.sourceProvider !== 'manual'
...
Analyze and report your findings.${memoryContext}${repoContext}`;
    ...
    const { handler: checkpointHandler, cleanup: checkpointCleanup } = createCheckpointToolHandler(
      channel,
      incidentId,
    );
    const toolHandler = async (name: string, toolInput: Record<string, unknown>) => {
      ...
      try {
        ...
        channel?.sendProgress({
          stage: 'tool_error',
          message: JSON.stringify({ toolName: name, error: errMsg.slice(0, 300) }),
        });
    ...
    await this.eventBus.publish({
      ...
      payload: {
        ...
        message: `Orchestrator investigation started with ${mergedTools.length} tools`,
      },
    });
    channel?.sendProgress({
      stage: 'orchestrator_started',
      message: `Investigation started with ${mergedTools.length} tools`,
      maxTurns: ORCHESTRATOR_CONFIG.maxTurns,
    });

    // 9. Run orchestrator
    logger.info(
      { incidentId, tenantId, toolCount: mergedTools.length, model: ORCHESTRATOR_CONFIG.model },
    ...
    for (const tc of result.toolCalls) {
      ...
      const output =
        tc.output.length > MAX_OUTPUT_BYTES
      ...
      try {
        ...
        logger.warn(
          { err, incidentId, toolCallId: tc.id, name: tc.name },
          'Failed to persist raw tool call — continuing',
        );
      }
    }
    logger.info(
      { incidentId, persisted: persistedCount, total: result.toolCalls.length },
      'Raw tool calls persisted',
    );
    ...
    channel?.sendProgress({
      ...
      message: 'Persisting investigation evidence...',
    });
    ...
    if (evidences.length === 0) {
      ...
      const reinvokeResult = await this.agentRunner.run({
        ...
        tools: this.buildToolsForAgent(
          ORCHESTRATOR_CONFIG,
          { hasAws: false, composioApps: [], hasRelay: false },
          [],
        ),
    ...
    channel?.sendProgress({
      stage: 'synthesizing',
      message: 'Synthesizing findings into root cause analysis...',
    });
    await this.eventBus.publish({
      eventType: 'investigation.progress',
      occurredAt: new Date().toISOString(),
...
${buildAgentSummary(result.response, result.toolCalls)}`;
    ...
    try {
      ...
      logger.warn(
        { err, incidentId },
        'Synthesis exhausted all citation retries — terminating as inconclusive',
      );
      return await this.terminateInconclusive({
        tenantId,
        incidentId,
      ...
      try {
        await this.eventBus.publish({
          ...
          payload: {
            ...
            stage: 'wave_started',
            wave: 3,
            message: 'Verification started: diagnosis_verifier',
          },
        });
        const verificationPrompt = `Diagnosis to verify:\nRoot Cause: "${investigationResult.potentialRootCause}"\n\nFindings:\n${investigationResult.findings
          .slice(0, 5)
        ...
        await this.eventBus.publish({
          ...
          payload: {
            ...
            stage: 'wave_completed',
            wave: 3,
            message: 'Verification completed: diagnosis_verifier',
          },
        });
      } catch (err) {
        logger.warn(
    ...
    const verificationCostUsd = successfulResults
      .filter((r) => r.wave === 3)
      .reduce((sum, r) => sum + r.costUsd, 0);
    ...
    logger.info(
      {
        ...
        totalCostUsd,
        investigationDurationMs,
        orchestratorCostUsd: orchestratorResult.costUsd,
        synthesisCostUsd,
        toolCalls: result.toolCalls.length,
        turns: result.turns,
      },
  ...
  ): Promise<{ successful: SubAgentResult[]; failed: Array<{ role: string; error: string }> }> {
      ...
      if (this.credentialVendor) {
        try {
          ...
          logger.info(
            ...
            'Credentials vended for sub-agent',
          );
        } catch (vendErr) {
          logger.warn(
            { role, tenantId, err: vendErr instanceof Error ? vendErr.message : String(vendErr) },
            'Credential vend failed — using stub credentials',
          );
          cloudCredentials = { provider: 'stub', credentials: {}, region: this.defaultRegion };
        }
      } else {
...
Analyze and report your findings.${memoryContext}${repoContext}${priorFindings ?? ''}`;
      ...
      logger.info(
        {
          ...
          hasAws: capabilities.hasAws,
          composioApps: capabilities.composioApps.length,
        },
        'Sub-agent starting',
      );
      const result = await this.agentRunner.run({
      ...
      logger.info(
        { role, incidentId, tenantId, toolCalls: result.toolCalls.length, costUsd: result.costUsd },
        'Sub-agent completed',
      );
    ...
    for (let idx = 0; idx < settledResults.length; idx++) {
      ...
      if (result.status === 'fulfilled') {
        ...
        const collapsed = collapseToolSummaries(allToolCalls.get(roles[idx]!) ?? []);
        const timelineSummary = formatCollapsedTimeline(collapsed);
        await this.eventBus.publish({
          eventType: 'investigation.progress',
          occurredAt: new Date().toISOString(),
