...
import { COMPOSIO_APP_MAP, COMPOSIO_PROVIDERS } from '../../../shared/infra/integrations/composio-client.js';
...
let composioAppsCache: Map<string, { name: string; description: string; logo: string; categories: string[] }> | null = null;
...
async function getComposioApps(): Promise<typeof composioAppsCache> {
    ...
    try {
        ...
        const map = new Map<string, { name: string; description: string; logo: string; categories: string[] }>();
        ...
        logger.warn({ err: err instanceof Error ? err.message : err }, 'Failed to fetch Composio apps catalog');
        return null;
    }
}

const DISPLAY_NAMES: Record<string, string> = {
    aws: 'Amazon Web Services', github: 'GitHub', gitlab: 'GitLab',
    bitbucket: 'Bitbucket', slack: 'Slack', teams: 'Microsoft Teams',
    discord: 'Discord', jira: 'Jira', linear: 'Linear', trello: 'Trello',
    shortcut: 'Shortcut', clickup: 'ClickUp', asana: 'Asana',
    datadog: 'Datadog', sentry: 'Sentry', pagerduty: 'PagerDuty',
    newrelic: 'New Relic', notion: 'Notion', confluence: 'Confluence',
    hubspot: 'HubSpot', zendesk: 'Zendesk', intercom: 'Intercom',
};
...
export function createIntegrationRoutes(useCases: IntegrationUseCases) {
    ...
    app.post('/test-connection', requireRole('admin'), async (c) => {
        ...
        if (!roleArn) {
            return c.json({ success: false, message: 'No AWS integration found for this tenant. Connect AWS first.' });
        }
        try {
            ...
            return c.json({ success: true, message: 'Connection successful. CauseFlow can access your AWS account.' });
        }
        catch (err) {
            const errName = err instanceof Error ? err.name : 'Unknown';
            const msg = err instanceof Error ? err.message : 'Unknown error';
            logger.warn({ err: msg, errName, tenantId: String(tid), roleArn }, 'test-connection AssumeRole failed');
            return c.json({ success: false, message: `Connection failed: ${errName === 'AccessDenied' ? 'AccessDenied — check the role trust policy includes ExternalId=' + String(tid) : msg}` });
        }
    });
    ...
    app.post('/connect', requireRole('admin'), async (c) => {
        ...
        if (!COMPOSIO_PROVIDERS.has(body.provider)) {
            throw new ValidationError(`Unsupported provider: ${body.provider}. Supported: ${[...COMPOSIO_PROVIDERS].join(', ')}`);
        }
    ...
    app.get('/', async (c) => {
        ...
        const bestByProvider = new Map<string, { provider: string; status: string; createdAt?: string }>();
        for (const cc of composioConnections) {
            if (!cc.provider || cc.provider.trim() === '') continue;
            const existing = bestByProvider.get(cc.provider);
            if (!existing || (STATUS_PRIORITY[cc.status] ?? 0) > (STATUS_PRIORITY[existing.status] ?? 0)) {
        ...
        const composioOnly = [...bestByProvider.values()]
            ...
            name: DISPLAY_NAMES[cc.provider] ?? cc.provider.charAt(0).toUpperCase() + cc.provider.slice(1),
            status: cc.status,
            source: 'composio',
