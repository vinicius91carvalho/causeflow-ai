...
import { pgDelete, pgGet, pgInsert, pgQuery, pgUpdate } from '../../../shared/infra/db/postgres/pg-utils.js';
...
export class PgCodeKnowledgeRepository implements ICodeKnowledgeRepository {
    ...
    async listRepos(tenantId: TenantId): Promise<RepoNode[]> {
        const rows = await pgQuery('repo_nodes', 'tenant_id = $1', [tenantId], { orderBy: 'created_at DESC' });
    ...
    async getReposByService(tenantId: TenantId, serviceId: ServiceNodeId): Promise<RepoServiceMap[]> {
        const rows = await pgQuery(
            'repo_service_maps',
            'tenant_id = $1 AND data->>\'serviceId\' = $2',
            [tenantId, serviceId],
        );
    ...
    async listServiceEdges(tenantId: TenantId): Promise<ServiceEdge[]> {
        const rows = await pgQuery('service_edges', 'tenant_id = $1', [tenantId], { orderBy: 'created_at ASC' });
