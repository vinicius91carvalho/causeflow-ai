...
import { pgGet, pgInsert, pgUpdate, pgQueryJson } from '../../../shared/infra/db/postgres/pg-utils.js';
...
export class PgTenantRepository implements ITenantRepository {
    ...
    async update(tenantId: TenantId, data: Partial<Omit<Tenant, 'tenantId' | 'createdAt'>>): Promise<Tenant> {
    ...
    async listByOwner(ownerEmail: string, options?: ListOptions): Promise<{ items: Tenant[]; cursor?: string }> {
