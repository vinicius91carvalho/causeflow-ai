...
export class ExecuteRemediationUseCase {
  ...
  async execute(input: ExecuteRemediationInput): Promise<Remediation> {
    ...
    for (let i = 0; i < updatedSteps.length; i++) {
      ...
      try {
        if (step.action === 'create_fix_pr') {
          const prResult = await this.executeCreateFixPR(
            tenantId,
    ...
    // 7. Publish event
    const isRollback =
      Boolean(remediation.rollbackOf) || remediation.description.toLowerCase().includes('rollback');
  ...
  async executeCreateFixPR(
    tenantId: TenantId,
    incidentId: IncidentId,
    params: Record<string, unknown>,
  ): Promise<PullRequestInfo | undefined> {
    const proposedFix = params.proposedFix as ProposedFix | undefined;
    if (!proposedFix || proposedFix.files.length === 0) return undefined;
    ...
    for (const file of proposedFix.files) {
      ...
      await codeRepo.createOrUpdateFile(
        owner,
        repo,
        file.path,
        file.content,
        `fix(incident-${incidentId}): ${file.changeDescription}`,
        branchName,
        existingSha,
      );
    }
  ...
  buildPRBody(incidentId: IncidentId, fix: ProposedFix): string {
    const fileList = fix.files
      .map(
        (f: { path: string; changeDescription: string }) =>
          `- \`${f.path}\`: ${f.changeDescription}`,
      )
      .join('\n');
    const tests =
      fix.testSuggestions.length > 0
